...
-- TASK 4
DROP TRIGGER IF EXISTS employeeStreetInsert;
DELIMITER //
CREATE TRIGGER employeeStreetInsert
    before INSERT
    on employee_street
    for EACH ROW
BEGIN
    if ((new.employee_id not in (select id from employee))
        OR (new.street not in (select id from street))) then
        signal sqlstate '45000'
